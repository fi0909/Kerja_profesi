...
mqtt_CD = 2  # cooldown publish

# === KONFIGURASI WAKTU ===
MIN_WAIT_TIME = 5         # waktu tunggu sebelum lampu hijau
DELAY_GREEN_OFF = 3     # waktu kosong sebelum lampu kembali merah
...
cap = cv2.VideoCapture("Video/test_video4.mp4")
DISPLAY_WIDTH, DISPLAY_HEIGHT = 640, 640

# === VARIABEL STATUS ===
waiting_ids = {}     # id: waktu_masuk_area_tunggu
...
while True:
    ...
    for result in results:
        ...
        ids = np.arange(len(boxes))  # pakai index sebagai id
    ...
    # === GAMBARKAN AREA ===
    cv2.polylines(annotated, [np.array(area_wait)], isClosed=True, color=(0, 255, 255), thickness=2)
    cv2.polylines(annotated, [np.array(area_cross)], isClosed=True, color=(255, 255, 0), thickness=2)

    # === GAMBARKAN LAMPU PEJALAN ===
    if lampu_hijau_pejalan:
    ...
    cv2.imshow("Smart Pedestrian 2-Area", annotated)
